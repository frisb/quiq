...
Channel = __decorate([
    routing_controllers_1.Middleware({ type: 'before', priority: 0 })
], Channel);
    ...
    return new Promise(function (resolve) {
        switch (req.header('content-type')) {
            case 'application/json':
                body_parser_1.json({ verify: rawBodyBuffer })(req, res, resolve);
                break;
            case 'application/x-www-form-urlencoded':
                body_parser_1.urlencoded({ verify: rawBodyBuffer, extended: true })(req, res, resolve);
                break;
            default:
                resolve();
                break;
